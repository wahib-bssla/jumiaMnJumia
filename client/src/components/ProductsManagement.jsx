...
export default function ProductsManagement() {
  ...
  // For editing, we keep both the current editing product and its original values.
  ...
  // Filter subcategories when category changes (for new product)
  useEffect(() => {
  ...
  const getDiff = () => {
    ...
    for (const key in editingProduct) {
      // For specifications, do a simple JSON comparison.
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      // Create a diff object for fields that have changed.
      const diff = getDiff();
      // If new images are provided, include them in the diff.
      if (editFileInputRef.current && editFileInputRef.current.files.length > 0) {
        diff.images = images;
      }
      // Also ensure numbers are correctly formatted.
