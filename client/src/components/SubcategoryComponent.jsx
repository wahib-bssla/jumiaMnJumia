...
export default function SubcategoryComponent({ subId }) {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        setLoading(true);
        // Fetch products based on subId or however you determine the products for this subcategory
        ...
        {products.length > 0 ? (
          products.map((product) => (
            <ProductCard
              ...
              price={product.price} // Assuming price is a property in the product object
            />
          ))
        ) : (
